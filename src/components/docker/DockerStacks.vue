...
import { Search, Layers, Play, Square, RotateCw, AlertCircle, Upload, X, FileText } from '@lucide/vue'
  ...
  <div class="flex min-h-0 flex-1 flex-col">
    ...
    <div class="flex-1 overflow-y-auto">
      ...
      <template v-else>
        <!-- 列表表头 -->
        <div
          class="sticky top-0 z-10 grid grid-cols-[1.75rem_minmax(0,2fr)_minmax(0,1fr)_6.5rem] items-center gap-3 border-x border-transparent bg-card px-3 py-1.5 text-caption text-muted-foreground border-b border-border/30"
        >
          <span />
          <span>stack</span>
          <span>配置文件</span>
        ...
        <div
          v-for="s in filtered"
          :key="s.name"
          class="group grid grid-cols-[1.75rem_minmax(0,2fr)_minmax(0,1fr)_6.5rem] items-center gap-3 rounded-lg border border-border/50 bg-card/30 px-3 py-2 transition-all hover:border-primary/30 hover:bg-card/60 hover:ring-1 hover:ring-primary/10"
        >
