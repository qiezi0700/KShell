...
const [dockerSource, sshApiSource, commandSource, libSource] = await Promise.all([
...
test('Docker Registry 密码不再拼入远端命令或环境变量', () => {
  ...
  assert.match(
    dockerSource,
    /sshExecWithStdin\(sessionId, command, password\)/,
  )
})
...
test('Docker 安装 sudo 密码只通过 stdin 发送', () => {
  ...
  assert.match(
    dockerSource,
    /dockerInstallCommand\(opts\)[\s\S]*sshExecWithStdin\(sessionId, command, `[\s\S]*opts\.sudoPassword/,
  )
})
